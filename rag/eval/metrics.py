...
Usage::
    ...
    from rag.eval.metrics import score_with_ragas

    scores = score_with_ragas(results, langchain_llm=ChatOllama(model="llama3.2:3b"))

    # Fast embedding-only scoring
    from rag.eval.metrics import score_embedding_similarity

    sims = score_embedding_similarity(results, embedder=pipeline.embedder)
"""

import math

from rag.eval.types import EvalResult
...
def score_with_ragas(
    ...
    def _metric(cls):
        return cls(llm=judge) if judge else cls()

    has_ground_truth = any(r.sample.ground_truth for r in results)
    metrics = [
        _metric(Faithfulness),
        _metric(ResponseRelevancy),
    ]
    if has_ground_truth:
        metrics += [_metric(LLMContextRecall), _metric(FactualCorrectness)]
